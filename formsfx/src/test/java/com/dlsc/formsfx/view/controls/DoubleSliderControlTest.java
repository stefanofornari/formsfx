...
public class DoubleSliderControlTest extends AbstractFxTest {
    ...
    protected void init(Stage stage, javafx.scene.layout.BorderPane root) {
        ...
        field = Field.ofDoubleType(dp)
            .label("Slider")
            .render(() -> DoubleSliderControl.of(0, 1, 1, null));

        createForm(field);
    }

    @Test
    public void all_nodes_have_slider_style_class() {
        HBox p = (HBox)field.getRenderer().node;
        then(p.getStyleClass()).contains("double-slider-control");

        for (Node n: p.getChildren()) {
            then(n.getStyleClass()).contains("double-slider-control");
        }
    }

    @Test
    public void labelSpan_default_is_2() {
        DoubleSliderControl control = new DoubleSliderControl(0, 1, 1);
    ...
    public void label_uses_labelSpan_in_layout_when_columns_geq_3() {
        DoubleField testField = Field.ofDoubleType(0.5).label("Test").span(5);
        DoubleSliderControl control = new DoubleSliderControl(0, 1, 1);
        control.labelSpan(3);
    ...
    public void label_uses_labelSpan_from_field_in_layout_when_columns_geq_3() {
        DoubleField testField = Field.ofDoubleType(0.5).label("Test").span(5).labelSpan(4);
        DoubleSliderControl control = new DoubleSliderControl(0, 1, 1);
        control.setField(testField);
